...
class UtilityTransaction(models.Model):
    ...
    reference = fields.Char(string='??????', required=True, index=True)
    date = fields.Datetime(default=fields.Datetime.now, string='???????')
    ...
    customer_id = fields.Many2one('res.partner', string='??????')
    account_id = fields.Many2one('utility.customer', string='??????', index=True)
    meter_id = fields.Many2one('utility.meter', string='??????')
    pos_order_id = fields.Many2one('pos.order', string='??? ???? ?????')
    reversal_id = fields.Many2one('utility.reversal', string='???????')
    adjustment_id = fields.Many2one('utility.adjustment', string='???????')
    operator_id = fields.Many2one('res.users', string='??????')
