...
class SaleWorkflowProcess(models.Model):
    ...
    validate_order = fields.Boolean(string='تأكيد أمر البيع تلقائياً (Validate Order)')
    order_filter_domain = fields.Text(related='order_filter_id.domain', readonly=False, string="فلتر أوامر البيع")
    create_invoice = fields.Boolean(string='إنشاء الفاتورة تلقائياً (Create Invoice)')
    create_invoice_filter_domain = fields.Text(related='create_invoice_filter_id.domain', readonly=False, string="فلتر إنشاء الفواتير")
    validate_invoice = fields.Boolean(string='ترحيل الفاتورة تلقائياً (Validate Invoice)')
    validate_invoice_filter_domain = fields.Text(related='validate_invoice_filter_id.domain', readonly=False, string="فلتر ترحيل الفواتير")
    validate_picking = fields.Boolean(string='تأكيد وتسليم المخزون تلقائياً (Validate Picking)')
    ...
    sale_done = fields.Boolean(string='إغلاق أمر البيع تلقائياً (Sale Done)')
