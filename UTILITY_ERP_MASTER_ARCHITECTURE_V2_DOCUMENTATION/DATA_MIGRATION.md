...
**CURRENT V1:** migration staging models live inside `utility_core`; the repository does not use a standalone `utility_migration` addon. Staging is persistent, bounded, retryable, lock-aware, source-traceable, and protected from duplicate execution. Admin-only manual Run Now remains a test/operational exception where exposed; normal processing is background-driven.

Runtime rehearsal, exact cutover timing, and restore-backed migration proof remain **DEFERRED** until executed evidence exists.
