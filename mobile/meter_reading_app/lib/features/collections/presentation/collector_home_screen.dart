              ...
              data: (list) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      _AccountWithInvoices(account: list[index]),
                );
              },
            ),
          ),
...
class _AccountWithInvoices extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final unpaid = invoices.where((i) =>
        ...
        i.status == InvoiceStatus.partiallyPaid).length;
    final overdue =
    ...
    return Column(
      ...
      children: [
        ...
        _InvoiceSummaryBar(
            total: invoices.length, paid: paid, unpaid: unpaid, overdue: overdue),
        const SizedBox(height: 8),
        Padding(
          padding: const EdgeInsets.symmetric(vertical: 4),
          child: Text('فواتير المشترك',
        ...
        if (invoices.isEmpty)
          Padding(
            ...
            child: Center(
                child: Text('لا توجد فواتير لهذا المشترك',
                    style: Theme.of(context)
                        .textTheme
                        .bodyMedium
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            _SummaryChip(
                label: 'إجمالي', value: '$total', color: s.onSurfaceVariant),
            _SummaryChip(
                label: 'مسددة', value: '$paid', color: Colors.green.shade700),
            _SummaryChip(
                label: 'غير مسددة', value: '$unpaid', color: s.onSurfaceVariant),
            _SummaryChip(label: 'متأخرة', value: '$overdue', color: s.error),
          ],
        ),
      ),
    );
...
class _InvoiceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (badgeColor, badgeText) = switch (invoice.status) {
      ...
      InvoiceStatus.unpaid => (s.outline, 'غير مسدد'),
    };

    final period = '${invoice.dueDate.month.toString().padLeft(2, '0')}/${invoice.dueDate.year}';
