...
class UtilityMediaAsset(models.Model):
    ...
    client_reading_uuid = fields.Char('معرف القراءة من تطبيق الموبايل', index=True, copy=False)
    ...
    ], string='خادم التخزين الرقمي', default='filesystem', required=True, index=True)

    # ===== مراجع التخزين الخارجي / filesystem (Production Path) =====
    external_original_reference = fields.Char('مرجع التخزين الخارجي الأصلي', index=True)
    ...
    @api.depends(
        'external_original_reference', 'external_review_reference', 'external_thumbnail_reference',
        'storage_backend',
    )
