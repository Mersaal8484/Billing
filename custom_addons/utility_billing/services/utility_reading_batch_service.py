...
class UtilityReadingBatchService(models.AbstractModel):
    ...
    def process_batch(self, batch_id, chunk_size=500):
        ...
        if not batch.line_ids:
            ...
            for item in readings_raw:
                line_vals.append({
                    ...
                    'reading_purpose': item.get('reading_purpose', 'periodic'),
                    'client_reading_uuid': item.get('client_reading_uuid') or item.get('reading_uuid'),
                    'asset_uuid': item.get('asset_uuid'),
        ...
        for i in range(0, total_pending, chunk_size):
            ...
            for line in chunk:
                try:
                    res = self._process_single_batch_line(
                        batch,
                        line,
    ...
    @api.model
    def _process_single_batch_line(
        self,
        batch,
        line,
        media_assets_by_name,
